...
/******/ 	__webpack_require__.p = "scripts/colorNamer/dist";
/******/
...
var ntc = {
  ...
  name: function name(color) {
    ...
    var cl = -1,
        ...
        ndf2 = 0,
        ndf = 0;
    var checkOpacity = function checkOpacity(opacity) {
    ...
    if (color.length < 3 || color.length > 7) return ["#000000", "Invalid Color: " + color, false, -1];
    ...
    if (isRGB) {
      parseFloat(opacity) <= 1 ? opacity = parseFloat(opacity * 100) : opacity = parseFloat(opacity);
    }
    return cl < 0 ? ["#000000", "Invalid Color: " + color, false, -1] : ["#" + ntc.names[cl][0], !parseFloat(opacity) ? ntc.names[cl][1] : ntc.names[cl][1] + '-' + opacity, false, parseFloat(opacity) ? Math.abs(opacity / 100) : -1];
  },
  ...
  hsl: function hsl(color) {
    ...
    var rgb = [parseInt('0x' + color.substring(1, 3), 10) / 255, parseInt('0x' + color.substring(3, 5), 10) / 255, parseInt('0x' + color.substring(5, 7), 10) / 255];
  ...
  rgba: function rgba(color) {
    var colorsOnly, r, g, b, a;
    colorsOnly = color.substring(color.indexOf('(') + 1, color.lastIndexOf(')')).split(/,\s*/);
    r = parseInt(colorsOnly[0], 10);
    g = parseInt(colorsOnly[1], 10);
    b = parseInt(colorsOnly[2], 10);
    a = parseFloat(colorsOnly[3], 10);
    return [r, g, b, a];
  },
  ...
  mixArray: function mixArray(additionalArr, originArr) {
    var arr = originArr;
...
var _nameThatColorJs = __webpack_require__(0);

var srtnm = _nameThatColorJs.ntc.names,
    clrop,
    clr,
    rgb,
    alt,
    ...
    $colorop = $("#colorop"),
    $colorpick = $("#colorpick"),
    n_match = _nameThatColorJs.ntc.name("#B0BF1A"),
    n_rgb = n_match[0],
...
var ntc_main = {
  ...
  init: function init() {
    ...
    srtnm = _nameThatColorJs.ntc.names;
    // srtnm.sort(ntc_main.nameSort);
    clrop = [];
    for (var i = 0; i < srtnm.length; i++) {
      clr = srtnm[i][0];
      rgb = _nameThatColorJs.ntc.rgb("#" + clr);
    ...
    $colorinp.on('keyup keydown', ntc_main.inpColor).change(ntc_main.inpColor);
    $("#colorop").change(ntc_main.inpColorList);
    ntc_main.setWheel(window.location.hash.length == 7 ? window.location.hash : "#6195ED");
  },

  inpColor: function inpColor() {
    var clr = $colorinp.get(0).value;

    if (clr.substring(0, 1) == "#" && clr.length == 7) return ntc_main.setWheel(clr);
    if (clr.substring(0, 1) != "#" && clr.length == 6) return ntc_main.setWheel("#" + clr);
  },

  inpColorList: function inpColorList(e) {
    if (e.target.value != "") {
      return ntc_main.setWheel("#" + e.target.value);
    }
  },

  nameSort: function nameSort(a, b) {
    return a[1] > b[1] ? 1 : a[1] < b[1] ? -1 : 0;
  },

  setColor: function setColor(clr) {
    var rgb = _nameThatColorJs.ntc.rgb(clr);
    $("#colorbox").css({ backgroundColor: clr });
    $colorinp.get(0).value = clr.toUpperCase();
    $("#colorrgb").html("rgb(" + rgb[0] + ", " + rgb[1] + ", " + rgb[2] + ")");
    n_match = _nameThatColorJs.ntc.name(clr);
  ...
  setWheel: function setWheel(clr) {
    ntc_main.fb.setColor(clr);
  }

};

exports.ntc_main = ntc_main;
...
function remove_duplicates_es6(arr) {
  var s = new Set(arr);
  var it = s.values(); // returns elements in insertion order
...
function addCustomList(outColor, className) {
  ...
  colorsSet = [],
    ...
    if (isHEX) {
      ...
      val = _nameThatColorJs.ntc.rgb(item);
      //val[3] = 1;//add alpha=1
      console.log('isHEX', _nameThatColorJs.ntc.name(item)[3]);
      val[3] = _nameThatColorJs.ntc.name(item)[3] === false ? -1 : _nameThatColorJs.ntc.name(item)[3];
    }
    if (isRGBA || isRGB) {
      val = _nameThatColorJs.ntc.rgba(item);
    }
    if (isRGB) {
      console.log('isRGB', _nameThatColorJs.ntc.name(item)[3]);
      val[3] = _nameThatColorJs.ntc.name(item)[3] === false ? -1 : _nameThatColorJs.ntc.name(item)[3];
    }
    return val;
  },
      setRGB = function setRGB(item) {
    var val = undefined,
        //arr or string
    isHEX = item.includes('#'),
        isRGBA = item.includes('rgba' || 'RGBA'),
  ...
  //put li inside ul
  outColor.map(function (item) {
    var rgb = _nameThatColorJs.ntc.rgb(item),
        n_match = _nameThatColorJs.ntc.name(item),
    ...
    if (isRGB) {
      ...
      item.includes('rgba' || 'RGBA') ? rgb = item : rgb = _nameThatColorJs.ntc.rgb2hex(red, green, blue);
    }
    _hex = item.includes('#') ? item : _nameThatColorJs.ntc.rgb2hex(red, green, blue);
    _rgb = setRGB(item);
    _rgba = setRGBA(item);
  ...
  $ul.after('<button class="btn btn-sm btn-outline-info mb-3 js_to-scss">to scss</button>');
  ...
  var scssBtnHandler = function scssBtnHandler() {
    ...
    colorsSet.map(function (item) {
      var showVal = item.opacity === false ? item.hex : 'rgba(' + item.rgba + ')';
      $ulSCSS.append('<li class=\'custom_select__item list-unstyled\'>            \n              $' + Case.toSnakeCase(item.name.toLowerCase()) + ': ' + showVal + ';            \n        </li>');
    });
  };
...
var Case = {
  titleToCamelCase: function titleToCamelCase(string) {
    if (typeof dashed != "string" || dashed.length == 0) return "";
  ...
  toSnakeCase: function toSnakeCase(string) {
    ...
    return string.replace(/(\s\w)/g, function (m) {
      return "-" + m[1].toLowerCase();
    });
  },
  snakeCaseToCamelCase: function snakeCaseToCamelCase(dashed) {
    if (typeof dashed != "string" || dashed.length == 0) return "";
...
var _eventDetectNTCJs = __webpack_require__(1);

var _extractColorsJs = __webpack_require__(2);

$(document).ready(function () {
  _nameThatColorJs.ntc.init();

  _nameThatColorJs.ntc.names.sort(_eventDetectNTCJs.ntc_main.nameSort);
  _eventDetectNTCJs.ntc_main.init();

  $('.js_btn-add-text').on('click', _extractColorsJs.addText);
  $('.js_btn-set-colors').on('click', _extractColorsJs.createCustomList);
});
