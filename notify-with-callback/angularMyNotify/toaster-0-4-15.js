...
/*
* author pasha oleynyk.
* This is extension of AngularJS Toaster(Version: 0.4.15), here been added callbacks as function
*/
...
(function (window, document) {
  ...
  angular.module('toaster', []).constant(
    'toasterConfig', {
      ...
      'time-out': 200, // Set timeOut and extendedTimeout to 0 to make it sticky
      'icon-classes': {
        error: 'toast-error',
        info: 'toast-info',
        wait: 'toast-wait',
    ...
    'toasterService', [
      '$rootScope', 'toasterConfig', function ($rootScope, toasterConfig, $timeout) {
        ...
        this.addToInbox = function(currentToast){
          if(this._isDelete){console.log('this._isDelete is',this._isDelete); return};

          this.inbox.push(currentToast); 
          console.log('adding is finish');
        };
        this.removeById = function(currentToast){
          console.log('currentToast',currentToast);
          this.inbox.splice(currentToast, 1);
        };

        this.removeThisToast= function(){
          var currentToast = 0;
        ...
        this.pop = function (toastId, type, title, body, timeout, bodyOutputType, clickHandler, toasterId, showCloseButton,  onHideCallback) {
          if (angular.isObject(type)) {
            ...
            //console.log('type is not Object');
            this.toast = {
              toastId: toastId,
              type: type,
              title: title,
    ...
    'toasterEventRegistry', [
      '$rootScope','toasterService', function ($rootScope,toasterService) {
        ...
        //allToastInbox is tmp-array, should recive from server
        var allToastInbox = [
          {toastId: 'id-0',type: 'success', title: "title", body: 'Its address is https://google.com.',timeout: 15000, bodyOutputType: 'trustedHtml'},
          {toastId: 'id-1',
            type: 'success',
            title:"title",
            body: '<ul><li>Render html <button on-click="close('+'"ok"'+')">message</button></li></ul>',
            timeout: 25000,
            bodyOutputType: 'trustedHtml'},
          {toastId:'id-2',
            type:'error',
            title:"title",
            body: '<ul><li>Render html <button class="btn btn-primary" style="margin: 50px 0 0 150px;" ' +
            'ng-click="alertMy('+"some  alert"+')">some  alert</button></li></ul>',
            timeout: null,
            bodyOutputType: 'trustedHtml'},
          {toastId:'id-3',type:'wait',    title:"title", body: 'SOme text id-3',
            timeout:null},
          {toastId:'id-4',
            type:'warning',
            title:"title -4",
            ...
            //clickHandler:function () {return 'clickHandler';},
            toasterId: 'toasterId-4',
            showCloseButton:true,
            onHideCallback: callbackEvent.callbackFunc
          },
          {toastId:'id-5',type:'note',    title:"title", body: "text"}
        ];
        ...
        toasterFactory = {
          ...
          subscribeToNewToastEvent: function (onNewToast) {
            //console.log('push to newToastEventSubscribers');
          ...
          getByIdToasts: function (id) {
          console.log('return ',allToastInbox[id], 'id',id);
          return allToastInbox[id];
          }
        };
        return {
          ...
          allToastInbox: allToastInbox,
          callbackEvent: callbackEvent,
          getByIdToasts: toasterFactory.getByIdToasts

        };
      }]
  ).directive(
    'toasterContainer', [
      '$parse', '$rootScope', '$interval', '$sce', 'toasterConfig', 'toasterService', 'toasterEventRegistry',
      function ($parse, $rootScope, $interval, $sce, toasterConfig, toasterService, toasterEventRegistry) {
        return {
          replace: true,
          restrict: 'EA',
          ...
          link: function (scope, elm, attrs) {
            ...
            scope._onNewToast = function (event, toasterId, toastId) {
              ...
              if ((isUndefinedOrNull(scope.config.toasterId) && isUndefinedOrNull(toasterId)) || (!isUndefinedOrNull(scope.config.toasterId) && !isUndefinedOrNull(toasterId) && scope.config.toasterId == toasterId)) {
                addToast(toasterService.toast, toastId);
              }
            };
          ...
          template: '<div id="toast-container" ng-class="[config.position, config.animation]">' +
                    ...
                    '<div ng-class="config.title">{{toaster.id}} {{toaster.title}}</div>' +
